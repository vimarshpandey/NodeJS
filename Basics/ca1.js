const chalk = require('chalk');
const yargs = require('yargs');
const inpt = yargs.argv;

console.log("Ther gender is",inpt._[0])
console.log("Enter the first name: ",inpt._[1]);
console.log("Enter gender is",inpt._[2])
console.log("Enter the second name: ",inpt._[3]);
console.log("Enter gender is",inpt._[4])
console.log("Enter the third name: ",inpt._[5]);
console.log("Enter gender is",inpt._[6])
console.log("Enter the third name: ",inpt._[7]);
console.log("Enter gender is",inpt._[8])
console.log("Enter the third name: ",inpt._[9]);
console.log("Enter gender is",inpt._[10])
console.log("Enter the third name: ",inpt._[11]);
console.log("Enter gender is",inpt._[12])
console.log("Enter the third name: ",inpt._[13]);
console.log("Enter gender is",inpt._[14])
console.log("Enter the third name: ",inpt._[15]);
console.log("Enter gender is",inpt._[16])
console.log("Enter the third name: ",inpt._[17]);
console.log("Enter gender is",inpt._[18])
console.log("Enter the third name: ",inpt._[19]);

if(inpt._[0] === "m"){
    console.log(chalk.yellow.bgGreen(inpt._[1]));
}else{
    console.log(chalk.green.bgWhite(inpt._[1]));
}
if(inpt._[2] === "m"){
    console.log(chalk.yellow.bgGreen(inpt._[3]));
}else{
    console.log(chalk.green.bgWhite(inpt._[3]));
}
if(inpt._[4] === "m"){
    console.log(chalk.yellow.bgGreen(inpt._[5]));
}else{
    console.log(chalk.green.bgWhite(inpt._[5]));
}
if(inpt._[6] === "m"){
    console.log(chalk.yellow.bgGreen(inpt._[7]));
}else{
    console.log(chalk.green.bgWhite(inpt._[7]));
}
if(inpt._[8] === "m"){
    console.log(chalk.yellow.bgGreen(inpt._[9]));
}else{
    console.log(chalk.green.bgWhite(inpt._[9]));
}
if(inpt._[10] === "m"){
    console.log(chalk.yellow.bgGreen(inpt._[11]));
}else{
    console.log(chalk.green.bgWhite(inpt._[11]));
}
if(inpt._[12] === "m"){
    console.log(chalk.yellow.bgGreen(inpt._[13]));
}else{
    console.log(chalk.green.bgWhite(inpt._[13]));
}
if(inpt._[14] === "m"){
    console.log(chalk.yellow.bgGreen(inpt._[15]));
}else{
    console.log(chalk.green.bgWhite(inpt._[15]));
}
if(inpt._[16] === "m"){
    console.log(chalk.yellow.bgGreen(inpt._[17]));
}else{
    console.log(chalk.green.bgWhite(inpt._[17]));
}
if(inpt._[18] === "m"){
    console.log(chalk.yellow.bgGreen(inpt._[19]));
}else{
    console.log(chalk.green.bgWhite(inpt._[19]));
}